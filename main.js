...
// Criar ícone na bandeja do sistema (NOVO - Menu Bar Inteligente)
function createTray() {
  // Usar TrayManager com menu inteligente
  const TrayManager = require('./tray-manager');
  trayManager = new TrayManager(mainWindow, createWindow);
  trayManager.create();
  tray = trayManager.tray;
  return; // Usar novo sistema
...
// ============================================
// 🔔 IPC HANDLERS - MENU BAR / TRAY
// ============================================

// Atualizar dados do tray com tickets
ipcMain.on('update-tray-tickets', (event, ticketsData) => {
  if (trayManager) {
    trayManager.updateTickets(ticketsData);
    
    // Se há um ticket pendente para focar (após recriar janela), focar agora
    if (trayManager.pendingTicketFocus && mainWindow && !mainWindow.isDestroyed()) {
      const ticketKey = trayManager.pendingTicketFocus;
      console.log(`🎯 Dados carregados! Focando no ticket pendente: ${ticketKey}`);
      
      setTimeout(() => {
        mainWindow.webContents.send('focus-ticket', ticketKey);
        trayManager.pendingTicketFocus = null;
      }, 500); // Aguardar 500ms após os dados carregarem
    }
  }
});
